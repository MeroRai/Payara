...
public class SetCloudTraceConfiguration implements AdminCommand {
    ...
    @Param(name = "frequencyUnit", optional = true)
    private String frequencyUnit;

    @Param(name = "frequencyValue", optional = true)
    private String frequencyValue;

    @Param(name = "cloudEndpointUrl", optional = true)
    ...
    public void execute(AdminCommandContext acc) {
        final AdminCommandContext theContext = acc;
            ...
            ConfigSupport.apply(new SingleConfigCode<CloudTraceConfiguration>() {
                ...
                public Object run(CloudTraceConfiguration config) throws PropertyVetoException {
                    ...
                    if (frequencyUnit != null) {
                        config.setFrequencyUnit(frequencyUnit);
                    }
                    ...
                    if (cloudEndpointUrl != null) {
                        config.setCloudEndpointUrl(cloudEndpointUrl);
                    }
                    return null;
                }
        ...
        if (dynamic) {
            enableOnTarget(theContext);
        }

    }

    private void enableOnTarget(AdminCommandContext context) {
        CommandRunner runner = serviceLocator.getService(CommandRunner.class);
        ActionReport subReport = context.getActionReport().addSubActionsReport();
        CommandRunner.CommandInvocation inv;

        inv = runner.getCommandInvocation("enable-cloud-trace", subReport, context.getSubject());
